...
document.onkeydown = (e) => {
  if(e.key === 'Enter')
    getPing(addressField.value, countField.value)
}
...
function getPing(addressValue, countValue) {
  ...
  let url = `../api/v1/ping.php?host=${addressValue}&count=${countValue}`
  fetch(url)
    .then(res => res.json())
...
function loadPingHistory(ping) {
  const values = [ping.address, ping.transmitted, ping.received, ping.losted, ping.totalTimes]
  pingTableContent.insertAdjacentHTML('afterbegin', `<tr data-address="${ping.address}"><td>${values.join('</td><td>')}</td></tr>`)
  pingTableContent.firstChild.onclick = () => {
    const row = event.target.parentNode
    const address = row.dataset.address
    loadStatssInfo(address)
    loadTimePackets(statusValues[address].times)
    Array.from(pingTableContent.childNodes).map(r => r.classList.remove('selected'))
...
function loadStatssInfo(address) {
  ...
  const transmittedPercentage = [losted / transmitted * 100, received / transmitted * 100]
